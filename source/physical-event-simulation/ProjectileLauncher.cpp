...
ProjectileLauncher::ProjectileLauncher(ch::AABB zone) :
	projectile_(ch::Circle({ zone.pos.x + zone.size.x, zone.pos.y + zone.size.y / 2.f }, P_OBJECTS_RADIUS), ch::AABB(zone.pos, { zone.size.x * 10.f, zone.size.y }), true),
	slingshot_(zone, projectile_)
{
	...
	topLeftInformations_.init(ch::AABB(zone.pos + ch::vec_t{ TEXT_MARGIN, TEXT_MARGIN }, { 200.f, 80.f }), 20u, "informations\nsur\nplusieurs\nlignes");
...
CircleRigidBody ProjectileLauncher::constructCircleRigidBody() const {
	return CircleRigidBody(projectile_.getCircle(), projectileMass_, slingshot_.getProjectileInitialVelocityInMetersPerSecond() * slingshot_.getProjectileDirection() * PIXELS_PER_METER * SLINGSHOT_SPEED_MULTIPLIER);
}
